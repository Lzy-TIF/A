...
using namespace std;
int main () {
	double $=500,max$=10000,am1=0,am2=0,am3=0,al3=0,al4=0,al5=0,wh1=0,wh2=0,wh3=0,al1=0,al2=0,pg1=0,pg2=0,up=0,a=65;
	while (1 == 1) {
		...
		cout<<"]";
		cout<<"	["<<$/100<<"/"<<max$/100<<"]"<<"			v1.2"<<endl<<endl;
		cout<<"----------------------------------------------------------------------"<<endl;
		if (up == 0) {
			cout<<"购买："<<endl; 
			cout<<"[0.升级面板]	[1.自动(1) (10$)] 	[2.仓库(1) (100$)]"<<endl<<endl;
		} else if (up == 1) {
			cout<<"升级 - 选择面板："<<endl;
			cout<<"[0.退出面板]	[1.自动点击]";
			if (al4 != 0) {
			...
			cout<<"升级 - 自动点击："<<endl;
			if (am1 == 0) {
				cout<<"[0.退出升级]	[1.依次升级(3) (100$)]	[2.全部升级(3) ("<<am2*100<<"$)]"<<endl;
			} else if (am1 != 0) {
				cout<<"[0.退出升级]	[1.依次升级(2) (20$)]	[2.全部升级(2) ("<<am1*20<<"$)]"<<endl;
			}
		} else if (up == 3) {
			cout<<"升级 - 仓库："<<endl;
			if (wh1 == 0) {
				cout<<"[0.退出升级]	[1.依次升级(3) (1000$)]	[2.全部升级(3) ("<<wh2*1000<<"$)]"<<endl;
			} else if (wh1 != 0) {
				cout<<"[0.退出升级]	[1.依次升级(2) (200$)]	[2.全部升级(2) ("<<wh1*200<<"$)]"<<endl;
			}
		}
		cout<<"----------------------------------------------------------------------"<<endl;
		cout<<"成就："<<endl;
		if (al1 == 0) {
			cout<<"[1.空间不足！ - 仓库(1)*2] - 奖励50$"<<endl;	
		} else {
			cout<<"[1.空间不足！ - 仓库(1)*2] - 已完成"<<endl;
		}
		if (al2 == 0) {
			cout<<"[2.更高级的马达 - 自动(2)*1] - 奖励自动(1)*5"<<endl;
		} else {
			cout<<"[2.更高级的马达 - 自动(2)*1] - 已完成"<<endl;
		}
		if (al3 == 0) {
			cout<<"[3.更多的钱 - 5000$] - 奖励 $*2;"<<endl;
		} else {
			cout<<"[3.更多的钱 - 5000$] - 已完成"<<endl;
		}
		if (al4 == 0) {
			cout<<"[4.流水化工厂 - 自动(2)*200] - 奖励解锁仓库(2)"<<endl;
		} else {
			cout<<"[4.流水化工厂 - 自动(2)*200] - 已完成"<<endl;
		}
		if (al5 == 0) {
			cout<<""<<endl;
		} else {
